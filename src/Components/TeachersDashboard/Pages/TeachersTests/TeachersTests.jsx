...
import TeacherTestsidebar from "./TeacherTestsidebar/TeacherTestsidebar";
...
const initialData = [
  { id: 1, test: "Test 1", createdDate: "2024-01-15", status: "Not Published", lastModified: "2 days ago by You", duration: 60, description: "Sample test 1", instructions: "Follow the guidelines", trashed: false, archived: false },
  { id: 2, test: "Test 2", createdDate: "2024-01-10", status: "Not Published", lastModified: "10 hours ago by You", duration: 45, description: "Sample test 2", instructions: "Read carefully", trashed: false, archived: false },
  { id: 3, test: "Test 3", createdDate: "2024-01-05", status: "Not Published", lastModified: "5 days ago by You", duration: 90, description: "Sample test 3", instructions: "Complete all sections", trashed: false, archived: false },
  { id: 4, test: "Test 4", createdDate: "2024-01-18", status: "Not Published", lastModified: "8 hours ago by You", duration: 30, description: "Sample test 4", instructions: "Time-bound test", trashed: false, archived: false },
  { id: 5, test: "Test 5", createdDate: "2024-01-12", status: "Not Published", lastModified: "3 days ago by You", duration: 75, description: "Sample test 5", instructions: "Answer all questions", trashed: false, archived: false },
];
...
const TeachersTests = () => {
  ...
  const [tags, setTags] = useState([
    { id: 1, name: "Important", color: "#FF0000", questions: [1, 3, 5] },
    { id: 2, name: "Review", color: "#FF9900", questions: [2, 4, 6] },
    { id: 3, name: "Completed", color: "#008000", questions: [7, 9] },
  ]);
    ...
    if (searchQuery) {
      result = result.filter((test) => {
        return (
          ...
          test.createdDate.toLowerCase().includes(searchQuery.toLowerCase())
        )
      })
    }
  ...
  useEffect(() => {
    localStorage.setItem("teacherTests", JSON.stringify(data));
    localStorage.setItem("teacherTestTags", JSON.stringify(tags));
  }, [data, tags]);
  ...
  const handleArchiveTest = (testId) => {
    setData(prevData =>
      prevData.filter(test =>
        !testId.some(st => st.id === test.id)
      )
    )

    const archivedTest = data.find(test => test.id === testId);
    if (archivedTest) {
      const archivedTags = JSON.parse(localStorage.getItem("teacherArchivetags") || "[]");
      const alreadyArchived = archivedTags.some(test => test.id === testId);
      if (!alreadyArchived) {
        localStorage.setItem("teacherArchivetags", JSON.stringify([...archivedTags, { ...archivedTest, archived: true, lastModified: new Date().toISOString() }]));
      }
    }
  };
  ...
  const handleDeleteTest = (testId) => {
    ...
    if (trashedTest) {
      const trashedTags = JSON.parse(localStorage.getItem("teacherTrashedTags") || "[]");
      const alreadyTrashed = trashedTags.some(test => test.id === testId);
      if (!alreadyTrashed) {
        localStorage.setItem("teacherTrashedTags", JSON.stringify([...trashedTags, { ...trashedTest, deleted: true, lastModified: new Date().toISOString() }]));
      }
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      width: "100px",
      cell: (row) => <div>{new Date(row.createdDate).toLocaleDateString()}</div>
    },
    // {
    //   name: "Status",
    //   selector: "status",
    ...
    // },
    {
      ...
      sortable: true,
      width: "100px",
      cell: (row) => <div>{row.lastModified}</div>
    },
    {
      name: "Actions",
      selector: "actions",
  ...
  return (
    <>
      <Helmet>
        <title>My Tests - Teacher</title>
        <meta name="description" content="Teacher Tests Dashboard" />
      </Helmet>
      ...
      <div className="test-index-wrapper">
        ...
        <div className="test-index-header-moblie">
          <h1 className="breadcrumb">My Tests</h1>
        ...
        <div className="test-index-container">
          <div className="test-index-header">
            <h1 className="breadcrumb desktop-title">Tests</h1>

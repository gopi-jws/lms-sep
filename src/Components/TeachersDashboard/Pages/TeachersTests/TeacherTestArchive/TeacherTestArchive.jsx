...
const TeacherTestArchive = () => {
  const [data, setData] = useState(getArchivedData());
  useEffect(() => {
    const handleStorageChange = () => {
      setData(getArchivedData());
    };
  ...
  //unarchive function
  const unarchiveTest = (row) => {
    // Find the test in "tags" localStorage and update archived to false
    ...
    // Remove from "archivetags" localStorage
    const archived = getArchivedData();
    const updatedArchived = archived.filter(item => item.id !== row.id);
    localStorage.setItem("archivetags", JSON.stringify(updatedArchived));
    setData(updatedArchived);
  }
...
export default TeacherTestArchive;